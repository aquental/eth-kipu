...
contract Modulo {
    ...
    mapping(address => uint256) public s_saldo;

    event Modulo_DepositoFeitoEUsuarioRegistrado(
        address depositante,
        uint256 valor
    );
    ...
    modifier onlyOwner() {
        if (msg.sender != i_owner)
            revert Modulo_CallerNotAllowed(msg.sender, i_owner);
        _;
    }

    modifier onlyOperators() {
        if (op.operador != msg.sender || op.isAllowed == false)
            revert Modulo_CallerNotAllowed(msg.sender, op.operador);
        _;
    }
    ...
    function withdraw(address _receiver, uint256 _amount)
        ...
        onlyOperators
    {
        require(_receiver != address(0), "Endereco Invalido");
        require(_amount >= ONE_ETHER, Modulo_AmountTooLow(_amount, ONE_ETHER));
        if (_amount > address(this).balance)
            revert Modulo_NotEnoughBalance(_amount, address(this).balance);
